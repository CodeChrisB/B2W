...
namespace B2W.TopDown
{
    public class PlayerCharacter : MonoBehaviour
    {
        ...
        [Header("Movement")]
        public float move_accel = 2f;
        public float move_deccel = 2f;
        public float move_max = 2f;
        ...
        public void SpinPlayer(int rounds,int time,bool lockPlayer)
        {
            //LeanTween.rotateAround(this.gameObject,new Vector3(0, 360 * 5,0),1f, 5f).setEaseInExpo();
            if (lockPlayer)
                disable_controls = true;
            LeanTween.rotateAroundLocal(this.gameObject, Vector3.up, 360f * rounds, time).setEaseInCubic().setOnComplete(() => { 
                if (lockPlayer) 
